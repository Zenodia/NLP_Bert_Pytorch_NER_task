...
def convert_examples_to_features(data, label_list, max_seq_length, tokenizer):
    ...
    ex_index=0
    n=len(data)
    for (textlist,labellist) in data:
        #print("examples inside convert_examples_to_features funcitons",type(examples))
        tokens = []
        labels = []
        valid = []
        label_mask = []
...
def main():
    ...
    label_list = ["O", "PER", "ORG", "LOC", "MISC", "PRG", "[CLS]", "[SEP]"]
    ...
    if args.do_train:
        ...
        #print(" ################### train_example type", type(train_examples), train_examples[0])
        train_examples=readfile(args.data_dir)
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        test_examples=readfile(args.data_dir)
        eval_features = convert_examples_to_features(test_examples, label_list, args.max_seq_length, tokenizer)
